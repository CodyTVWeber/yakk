...
from yakk.tools.converse import converse as converse_tool
...
class TestSpeedParameter:
    ...
    async def test_speed_accepts_float(self):
        ...
        with patch('yakk.tools.converse.startup_initialization', new_callable=AsyncMock):
            with patch('yakk.tools.converse.text_to_speech_with_failover', new_callable=AsyncMock) as mock_tts:
    ...
    async def test_speed_none_is_valid(self):
        ...
        with patch('yakk.tools.converse.startup_initialization', new_callable=AsyncMock):
            with patch('yakk.tools.converse.text_to_speech_with_failover', new_callable=AsyncMock) as mock_tts:
                with patch('yakk.tools.converse.TTS_SPEED', None):
                    mock_tts.return_value = (True, {'generation': 1.0, 'playback': 2.0}, {})
    ...
    async def test_speed_reads_from_config(self):
        ...
        with patch('yakk.tools.converse.startup_initialization', new_callable=AsyncMock):
            with patch('yakk.tools.converse.text_to_speech_with_failover', new_callable=AsyncMock) as mock_tts:
                with patch('yakk.tools.converse.TTS_SPEED', 1.3):
                    mock_tts.return_value = (True, {'generation': 1.0, 'playback': 2.0}, {})
    ...
    async def test_speed_config_out_of_range_error(self):
        ...
        with patch('yakk.tools.converse.startup_initialization', new_callable=AsyncMock):
            with patch('yakk.tools.converse.TTS_SPEED', 10.0):  # Invalid value
                result = await converse(
                    message="Test",
                    wait_for_response=False,
                    speed=None
    ...
    async def test_speed_explicit_out_of_range_no_env_mention(self):
        ...
        with patch('yakk.tools.converse.startup_initialization', new_callable=AsyncMock):
            with patch('yakk.tools.converse.TTS_SPEED', 1.5):  # Valid config value
                result = await converse(
                    message="Test",
                    wait_for_response=False,
