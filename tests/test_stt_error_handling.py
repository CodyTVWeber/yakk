...
class TestSTTErrorHandling:
    ...
    async def test_connection_refused_all_endpoints(self):
        ...
        with patch('yakk.simple_failover.STT_BASE_URLS', TEST_STT_BASE_URLS):
            with patch('yakk.simple_failover.AsyncOpenAI') as MockClient:
    ...
    async def test_no_api_key_error(self):
        ...
        with patch('yakk.simple_failover.AsyncOpenAI') as MockClient:
            ...
            with patch('yakk.simple_failover.OPENAI_API_KEY', None):
                result = await simple_stt_failover(mock_file)
