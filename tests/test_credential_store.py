"""
Unit tests for yakk.credential_store module.
...
import yakk.credential_store as cred_mod
from yakk.credential_store import (
    KEYRING_SERVICE,
    KEYRING_USERNAME,
    KeyringStore,
    PlaintextStore,
...
class TestPlaintextStore:
    ...
    def temp_credentials_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("yakk.credential_store.CREDENTIALS_DIR", cred_dir)
        monkeypatch.setattr("yakk.credential_store.CREDENTIALS_FILE", cred_file)
...
class TestGetCredentialStore:
    ...
    def test_keyring_when_configured_and_viable(self, monkeypatch):
        ...
        with patch("yakk.credential_store._keyring_backend_is_viable", return_value=True), \
             patch("yakk.credential_store._migrate_plaintext_to_keyring"):
            store = get_credential_store()
            assert isinstance(store, KeyringStore)

    def test_fallback_to_plaintext_when_keyring_unavailable(self, monkeypatch):
        monkeypatch.setenv("YAKK_CREDENTIAL_STORE", "keyring")
        with patch("yakk.credential_store._keyring_backend_is_viable", return_value=False):
            store = get_credential_store()
            assert isinstance(store, PlaintextStore)
