...
class TestMinDurationIntegration:
    ...
    async def test_converse_validates_min_duration(self):
        ...
        with patch('yakk.tools.converse.startup_initialization', new_callable=AsyncMock):
            ...
            import yakk.config
            yakk.config.FFMPEG_AVAILABLE = True
            ...
            with patch('yakk.tools.converse.logger') as mock_logger:
                with patch('yakk.tools.converse.text_to_speech_with_failover', new_callable=AsyncMock) as mock_tts:
                    mock_tts.return_value = (True, {}, {})
                    with patch('yakk.tools.converse.record_audio_with_silence_detection') as mock_record:
                        mock_record.return_value = (np.array([1, 2, 3]), True)  # Returns tuple (audio, speech_detected)
                        with patch('yakk.tools.converse.speech_to_text', new_callable=AsyncMock) as mock_stt:
