...
from yakk.tools.service import service as service_tool
service = getattr(service_tool, 'fn', service_tool)

# Import prompts for testing
from yakk.prompts.services import whisper_prompt as whisper_prompt_tool
from yakk.prompts.services import kokoro_prompt as kokoro_prompt_tool
...
class TestUnifiedServiceTool:
    ...
    async def test_status_service_not_running(self):
        ...
        with patch('yakk.tools.service.check_service_status', return_value=("not_available", None)):
    ...
    async def test_status_service_running(self):
        ...
        with patch('yakk.tools.service.check_service_status', return_value=("local", mock_proc)), \
    ...
    async def test_start_service_already_running(self):
        ...
        mock_proc = MagicMock()
        with patch('yakk.tools.service.find_process_by_port', return_value=mock_proc):
    ...
    async def test_start_whisper_service(self):
        ...
        with patch('yakk.tools.service.find_process_by_port', side_effect=[None, MagicMock()]), \
             patch('yakk.tools.service.find_whisper_server', return_value="/path/to/whisper-server"), \
             patch('yakk.tools.service.find_whisper_model', return_value="/path/to/model.bin"), \
    ...
    async def test_start_whisper_missing_binary(self):
        ...
        with patch('yakk.tools.service.find_process_by_port', return_value=None), \
             patch('yakk.tools.service.find_whisper_server', return_value=None), \
             patch('pathlib.Path.exists', return_value=False):
            result = await service("whisper", "start")
            assert "❌" in result
    ...
    async def test_restart_service(self):
        ...
        with patch('yakk.tools.service.stop_service', return_value="✅ Service stopped"), \
             patch('yakk.tools.service.start_service', return_value="✅ Service started"), \
             patch('asyncio.sleep'):
    ...
    async def test_enable_service_macos(self):
        ...
        with patch('platform.system', return_value='Darwin'), \
             patch('yakk.tools.service.load_service_template', return_value="template content") as mock_template, \
             patch('yakk.tools.service.find_whisper_server', return_value="/Users/test/.yakk/services/whisper/build/bin/whisper-server"), \
             patch('yakk.tools.service.find_whisper_model', return_value="/path/to/model.bin"), \
...
<dict>
    ...
    async def test_enable_service_linux(self):
        ...
        with patch('platform.system', return_value='Linux'), \
             patch('yakk.tools.service.load_service_template', return_value=mock_template_content), \
             patch('yakk.tools.service.find_kokoro_fastapi', return_value="/path/to/kokoro"), \
             patch('pathlib.Path.exists', return_value=True), \
             patch('pathlib.Path.mkdir'), \
             patch('pathlib.Path.write_text'), \
             patch('subprocess.run') as mock_run:
