...
from yakk.cli import yakk_main_cli
...
class TestCloneGroupRegistered:
    ...
    def test_clone_group_in_main_cli(self, runner):
        ...
        result = runner.invoke(yakk_main_cli, ["clone", "--help"])
    ...
    def test_clone_no_longer_has_service_subcommands(self, runner):
        ...
        for op in ("install", "status", "uninstall"):
            result = runner.invoke(yakk_main_cli, ["clone", op, "--help"])
...
class TestCloneAdd:
    ...
    def test_add_help(self, runner):
        result = runner.invoke(yakk_main_cli, ["clone", "add", "--help"])
        assert result.exit_code == 0
    ...
    @patch("yakk.tools.clone.profiles.clone_add", new_callable=AsyncMock)
    def test_add_success(self, mock_add, runner, tmp_path):
        ...
        result = runner.invoke(yakk_main_cli, [
...
class TestCloneList:
    ...
    def test_list_help(self, runner):
        result = runner.invoke(yakk_main_cli, ["clone", "list", "--help"])
        assert result.exit_code == 0
        assert "List available clone voice profiles" in result.output

    @patch("yakk.tools.clone.profiles.clone_list", new_callable=AsyncMock)
    def test_list_voices(self, mock_list, runner):
        ...
        result = runner.invoke(yakk_main_cli, ["clone", "list"])
        assert result.exit_code == 0
...
class TestCloneRemove:
    ...
    def test_remove_help(self, runner):
        result = runner.invoke(yakk_main_cli, ["clone", "remove", "--help"])
        assert result.exit_code == 0
        assert "NAME" in result.output
        assert "--keep-audio" in result.output

    @patch("yakk.tools.clone.profiles.clone_remove", new_callable=AsyncMock)
    def test_remove_success(self, mock_remove, runner):
        ...
        result = runner.invoke(yakk_main_cli, ["clone", "remove", "testvoice"])
        assert result.exit_code == 0
    ...
    def test_remove_not_found(self, mock_remove, runner):
        ...
        result = runner.invoke(yakk_main_cli, ["clone", "remove", "nonexistent"])
        assert result.exit_code == 1
    ...
    def test_remove_keep_audio(self, mock_remove, runner):
        ...
        result = runner.invoke(yakk_main_cli, ["clone", "remove", "testvoice", "--keep-audio"])
        assert result.exit_code == 0
    ...
    def test_remove_missing_name(self, runner):
        ...
        result = runner.invoke(yakk_main_cli, ["clone", "remove"])
        assert result.exit_code != 0
