...
async def test_stt_audio_saved_with_simple_failover():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('yakk.config.SAVE_ALL', True), \
             patch('yakk.config.SAVE_AUDIO', True), \
             patch('yakk.tools.converse.SAVE_AUDIO', True):

            # Mock the simple_stt_failover to return test transcription dict
            with patch('yakk.simple_failover.simple_stt_failover', new_callable=AsyncMock) as mock_stt:
                ...
                with patch('yakk.tools.converse.get_conversation_logger') as mock_logger:
