...
def tmp_yakk(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "yakk.tools.clone.profiles.VOICES_JSON", voices_json
    )
    monkeypatch.setattr(
        "yakk.tools.clone.profiles.VOICES_DIR", voices_dir
    )
...
class TestSaveVoicesJson:
    ...
    def test_creates_parent_directory(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("yakk.tools.clone.profiles.VOICES_JSON", nested)
...
class TestCloneAdd:
    ...
    async def test_add_with_auto_transcribe(self, tmp_yakk, sample_audio):
        ...
        with patch(
            "yakk.tools.clone.profiles._transcribe_audio",
...
class TestTranscribeAudio:
    ...
    def test_transcribe_connection_error(self, sample_audio):
        ...
        with patch(
            "yakk.tools.clone.profiles.urllib.request.urlopen",
    ...
    def test_transcribe_successful(self, sample_audio):
        ...
        with patch("yakk.tools.clone.profiles.urllib.request.urlopen", return_value=mock_response):
