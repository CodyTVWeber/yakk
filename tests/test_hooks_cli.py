...
from yakk.cli_commands.claude import (
    claude,
    get_available_hooks,
...
def temp_settings_dir(tmp_path, monkeypatch):
    ...
    from yakk.cli_commands import claude as claude_mod
...
class TestResolveHookCommand:
    ...
    def test_resolve_hook_command_installs_when_missing(self, tmp_path):
        ...
        with patch('shutil.which', return_value=None):
            with patch('yakk.cli_commands.claude.Path.home', return_value=fake_home):
                result = resolve_hook_command()
                assert str(home_bin) in result
                assert '|| true' in result
...
class TestHooksAddCommand:
    ...
    def test_add_all_hooks(self, runner, temp_settings_dir):
        ...
        with patch('yakk.cli_commands.claude.resolve_hook_command', return_value='yakk-hook-receiver || true'):
...
class TestGetInstalledHookNames:
    ...
    def test_handles_read_error(self):
        ...
        with patch('yakk.cli_commands.claude.read_settings', side_effect=Exception("file error")):
            installed = get_installed_hook_names('user')
        assert installed == set()
