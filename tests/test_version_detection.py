...
def test_get_version_non_git():
    ...
    with patch('yakk.version.is_git_repository', return_value=False):
        version = get_version()
...
def test_get_version_git_with_commit():
    ...
    with patch('yakk.version.is_git_repository', return_value=True):
        with patch('yakk.version.get_git_commit_hash', return_value='abc1234'):
