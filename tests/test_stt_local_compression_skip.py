...
from yakk.tools.converse import speech_to_text
from yakk import config
...
async def test_stt_skips_compression_for_local_endpoint():
    ...
    with patch('yakk.config.STT_BASE_URLS', ['http://127.0.0.1:2022/v1']), \
         patch('yakk.config.STT_COMPRESS', 'auto'), \
         patch('yakk.tools.converse.prepare_audio_for_stt', side_effect=capture_format), \
         patch('yakk.simple_failover.simple_stt_failover', new_callable=AsyncMock) as mock_stt:
...
async def test_stt_compresses_for_remote_endpoint():
    ...
    with patch('yakk.config.STT_BASE_URLS', ['https://api.openai.com/v1']), \
         patch('yakk.config.STT_COMPRESS', 'auto'), \
         patch('yakk.tools.converse.STT_AUDIO_FORMAT', 'mp3'), \
         patch('yakk.tools.converse.prepare_audio_for_stt', side_effect=capture_format), \
         patch('yakk.simple_failover.simple_stt_failover', new_callable=AsyncMock) as mock_stt:
...
async def test_stt_always_mode_compresses_local():
    ...
    with patch('yakk.config.STT_BASE_URLS', ['http://127.0.0.1:2022/v1']), \
         patch('yakk.config.STT_COMPRESS', 'always'), \
         patch('yakk.tools.converse.STT_AUDIO_FORMAT', 'mp3'), \
         patch('yakk.tools.converse.prepare_audio_for_stt', side_effect=capture_format), \
         patch('yakk.simple_failover.simple_stt_failover', new_callable=AsyncMock) as mock_stt:
...
async def test_stt_never_mode_skips_compression_for_remote():
    ...
    with patch('yakk.config.STT_BASE_URLS', ['https://api.openai.com/v1']), \
         patch('yakk.config.STT_COMPRESS', 'never'), \
         patch('yakk.tools.converse.prepare_audio_for_stt', side_effect=capture_format), \
         patch('yakk.simple_failover.simple_stt_failover', new_callable=AsyncMock) as mock_stt:
