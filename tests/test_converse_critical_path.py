...
class TestConverseOpenAIErrors:
    ...
    async def test_converse_reports_invalid_api_key_clearly(self):
        ...
        with patch('yakk.core.text_to_speech') as mock_tts:
            ...
            with patch('yakk.config.TTS_BASE_URLS', ['https://api.openai.com/v1']):
                with patch('yakk.config.OPENAI_API_KEY', 'invalid-key'):
                    result = await converse.fn(
                        message="Test message",
                        wait_for_response=False
                    )
...
class TestConverseFailoverBehavior:
    ...
    async def test_converse_tries_all_configured_endpoints(self):
        ...
        with patch('yakk.simple_failover.simple_tts_failover') as mock_tts:
            ...
            with patch('yakk.config.TTS_BASE_URLS', test_urls):
                result = await converse.fn(
                    message="Test message",
                    wait_for_response=False
                )
...
class TestConverseErrorMessages:
    ...
    async def test_error_message_suggests_checking_services(self):
        ...
        with patch('yakk.simple_failover.simple_tts_failover') as mock_tts:
            ...
            with patch('yakk.config.OPENAI_API_KEY', None):
                result = await converse.fn(
                    message="Test",
                    wait_for_response=False
                )
    ...
    async def test_stt_failure_reports_clearly(self):
        ...
        with patch('yakk.simple_failover.simple_tts_failover') as mock_tts:
            ...
            with patch('yakk.tools.converse.record_audio') as mock_record:
                ...
                with patch('yakk.simple_failover.simple_stt_failover') as mock_stt:
