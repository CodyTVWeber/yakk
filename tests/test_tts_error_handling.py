...
class TestTTSErrorHandling:
    ...
    async def test_connection_refused_all_endpoints(self):
        ...
        with patch('yakk.simple_failover.TTS_BASE_URLS', ['http://localhost:8880/v1', 'https://api.openai.com/v1']), \
             patch('yakk.simple_failover.OPENAI_API_KEY', 'test-key'), \
             patch('yakk.core.text_to_speech') as mock_tts:
    ...
    async def test_authentication_error_openai(self):
        ...
        with patch('yakk.simple_failover.TTS_BASE_URLS', ['https://api.openai.com/v1']), \
             patch('yakk.simple_failover.OPENAI_API_KEY', 'invalid-key'), \
             patch('yakk.core.text_to_speech') as mock_tts:
    ...
    async def test_no_api_key_error(self):
        ...
        with patch('yakk.simple_failover.TTS_BASE_URLS', ['https://api.openai.com/v1']), \
             patch('yakk.simple_failover.OPENAI_API_KEY', None):
    ...
    async def test_successful_tts(self):
        ...
        with patch('yakk.simple_failover.TTS_BASE_URLS', ['http://localhost:8880/v1']), \
             patch('yakk.simple_failover.OPENAI_API_KEY', None), \
             patch('yakk.core.text_to_speech') as mock_tts:
    ...
    async def test_fallback_to_openai(self):
        ...
        with patch('yakk.simple_failover.TTS_BASE_URLS', ['http://localhost:8880/v1', 'https://api.openai.com/v1']), \
             patch('yakk.simple_failover.OPENAI_API_KEY', 'valid-key'), \
             patch('yakk.core.text_to_speech') as mock_tts:
