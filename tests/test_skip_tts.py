...
@pytest.fixture
def yakk_server():
    """Create a yakk server for testing"""
    return mcp
...
class TestSkipTTS:
    ...
    @pytest.mark.asyncio
    async def test_skip_tts_true_overrides_env(self, yakk_server, original_skip_tts):
    ...
    @pytest.mark.asyncio
    async def test_skip_tts_false_overrides_env(self, yakk_server, original_skip_tts):
    ...
    @pytest.mark.asyncio
    async def test_skip_tts_none_follows_env_true(self, yakk_server, original_skip_tts):
        """Test that skip_tts=None follows environment variable when true"""
        # Set environment variable to true
        os.environ["YAKK_SKIP_TTS"] = "true"
        
        with patch("yakk.tools.converse.text_to_speech_with_failover") as mock_tts, \
             patch("yakk.tools.converse.SKIP_TTS", True):
            mock_tts.return_value = (True, {"ttfa": 1.0, "generation": 1.0, "playback": 2.0}, {"provider": "openai"})
    ...
    @pytest.mark.asyncio
    async def test_skip_tts_none_follows_env_false(self, yakk_server, original_skip_tts):
        """Test that skip_tts=None follows environment variable when false"""
        # Set environment variable to false
        os.environ["YAKK_SKIP_TTS"] = "false"
        
        with patch("yakk.tools.converse.text_to_speech_with_failover") as mock_tts, \
             patch("yakk.tools.converse.SKIP_TTS", False):
            mock_tts.return_value = (True, {"ttfa": 1.0, "generation": 1.0, "playback": 2.0}, {"provider": "openai"})
