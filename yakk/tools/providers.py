            ...
            for url in urls:
                if optimistic:
                    ...
                    provider_registry.registry[service][url] = EndpointInfo(
                        base_url=url,
                        models=_default_stt_models(url) if service == "stt" else ["tts-1"],
                        voices=[] if service == "stt" else ["af_alloy", "af_aoede", "af_bella", "af_heart", "af_jadzia", "af_jessica", "af_kore", "af_nicole", "af_nova", "af_river", "af_sarah", "af_sky", "am_adam", "am_echo", "am_eric", "am_fenrir", "am_liam", "am_michael", "am_onyx", "am_puck", "bm_daniel", "bm_fable", "bm_george", "bm_lewis"],
