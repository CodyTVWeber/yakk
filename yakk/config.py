"""
Configuration and shared utilities for Yakk Server.

This module contains all configuration constants, global state, initialization functions,
and shared utilities used across the yakk server.
"""
...
def find_yakk_env_files() -> list[Path]:
    """
    Find .yakk.env files by walking up the directory tree.
    
    Looks for (in order of priority - closest to current directory wins):
    1. .yakk.env in current or parent directories  
    2. .yakk/yakk.env in current or parent directories
    3. ~/.yakk/yakk.env in user home (global config)
    ...
    global_config = Path.home() / ".yakk" / "yakk.env"
    
    # Backwards compatibility: check for old filename
    if not global_config.exists():
        old_global = Path.home() / ".yakk" / ".yakk.env"
    ...
    while current_dir != current_dir.parent:
        # Check for standalone .yakk.env first
        standalone_file = current_dir / ".yakk.env"
        if standalone_file.exists():
            project_configs.append(standalone_file)
            break  # Stop at first found (closest wins)
            
        # Then check .yakk/yakk.env  
        dir_file = current_dir / ".yakk" / "yakk.env"
...
def load_yakk_env():
    """Load configuration from yakk.env files, with cascading from global to project-specific."""
    config_files = find_yakk_env_files()
    
    # If no config files found, create default global config
    if not config_files:
        default_path = Path.home() / ".yakk" / "yakk.env"
        default_path.parent.mkdir(parents=True, exist_ok=True)
        default_config = '''# Yakk Configuration File
...
# Base directory for all yakk data (default: ~/.yakk)
# YAKK_BASE_DIR=~/.yakk

# Models directory (default: ~/.yakk/models)
# YAKK_MODELS_DIR=~/.yakk/models

# Enable debug mode (true/false)
# YAKK_DEBUG=false

# Enable VAD debug logging (true/false)
# YAKK_VAD_DEBUG=false

# Save all audio and transcriptions (true/false)
# YAKK_SAVE_ALL=false

# Save audio files (true/false)
# YAKK_SAVE_AUDIO=false

# Save transcription files (true/false)
# YAKK_SAVE_TRANSCRIPTIONS=false
...
# YAKK_TOOLS_DISABLED=pronunciation_add,pronunciation_remove

# Examples:
# Minimal (just voice conversation): YAKK_TOOLS_ENABLED=converse
# Voice + config: YAKK_TOOLS_ENABLED=converse,service,config_get,config_set
# Load all tools: YAKK_TOOLS_DISABLED=
# All except pronunciation: YAKK_TOOLS_DISABLED=pronunciation_add,pronunciation_remove,pronunciation_list
...
# YAKK_TTS_BASE_URLS=http://127.0.0.1:8880/v1

# Comma-separated list of STT endpoints (local Whisper only)
# YAKK_STT_BASE_URLS=http://127.0.0.1:2022/v1
...
# Example: YAKK_STT_PROMPT=tmux, Tali, kubectl, Yakk
# YAKK_STT_PROMPT=
...
# YAKK_KOKORO_MODELS_DIR=~/.yakk/models/kokoro
...
# YAKK_CLONE_MODEL=mlx-community/Qwen3-TTS-12Hz-1.7B-Base-5bit  # ok, slightly larger
# YAKK_CLONE_MODEL=mlx-community/Qwen3-TTS-12Hz-1.7B-Base-6bit  # ok, slightly larger
# YAKK_CLONE_MODEL=mlx-community/Qwen3-TTS-12Hz-1.7B-Base-bf16  # full quality, slow (~1× realtime)
# YAKK_CLONE_MODEL=mlx-community/Qwen3-TTS-12Hz-0.6B-Base-5bit  # smaller (1.7GB), ok quality
...
# YAKK_DEFAULT_LISTEN_DURATION=120.0
...
# YAKK_EVENT_LOG_ENABLED=true

# Event log directory (default: ~/.yakk/logs/events)
# YAKK_EVENT_LOG_DIR=~/.yakk/logs/events

# Log rotation policy (currently only 'daily' supported)
# YAKK_EVENT_LOG_ROTATION=daily
...
# YAKK_PRONUNCIATION_ENABLED=true
...
# Add custom rules with YAKK_PRONOUNCE_* variables
# YAKK_PRONOUNCE_CUSTOM=

# Log pronunciation substitutions for debugging (true/false, default: false)
# YAKK_PRONUNCIATION_LOG_SUBSTITUTIONS=false
...
# plaintext stores in ~/.yakk/credentials (chmod 600)
# YAKK_CREDENTIAL_STORE=keyring
...
# Base directory for all yakk data
BASE_DIR = expand_path(os.getenv("YAKK_BASE_DIR", str(Path.home() / ".yakk")))

# Unified directory structure
AUDIO_DIR = BASE_DIR / "audio"
TRANSCRIPTIONS_DIR = BASE_DIR / "transcriptions"
LOGS_DIR = BASE_DIR / "logs"
# CONFIG_DIR = BASE_DIR / "config"  # Removed - config stored in .yakk.env file instead
MODELS_DIR = expand_path(os.getenv("YAKK_MODELS_DIR", str(BASE_DIR / "models")))

# Debug configuration
DEBUG = os.getenv("YAKK_DEBUG", "").lower() in ("true", "1", "yes", "on")
TRACE_DEBUG = os.getenv("YAKK_DEBUG", "").lower() == "trace"
VAD_DEBUG = os.getenv("YAKK_VAD_DEBUG", "").lower() in ("true", "1", "yes", "on")
DEBUG_DIR = LOGS_DIR / "debug"  # Debug files now go under logs

# Master save-all configuration
SAVE_ALL = os.getenv("YAKK_SAVE_ALL", "").lower() in ("true", "1", "yes", "on")

# Audio saving configuration
# Enable if SAVE_ALL is true, DEBUG is true, or individually enabled
SAVE_AUDIO = SAVE_ALL or DEBUG or os.getenv("YAKK_SAVE_AUDIO", "").lower() in ("true", "1", "yes", "on")
SAVE_TRANSCRIPTIONS = SAVE_ALL or DEBUG or os.getenv("YAKK_SAVE_TRANSCRIPTIONS", "").lower() in ("true", "1", "yes", "on")

# Audio feedback configuration
AUDIO_FEEDBACK_ENABLED = os.getenv("YAKK_AUDIO_FEEDBACK", "true").lower() in ("true", "1", "yes", "on")

# Skip TTS configuration (skip text-to-speech for faster responses)
SKIP_TTS = os.getenv("YAKK_SKIP_TTS", "false").lower() in ("true", "1", "yes", "on")

# TTS speed configuration (0.25-4.0, default None uses provider default)
TTS_SPEED = float(os.getenv("YAKK_TTS_SPEED")) if os.getenv("YAKK_TTS_SPEED") else None

# Metrics output level configuration (minimal/summary/verbose)
# - minimal: Just the response text, no timing
# - summary: Response + compact timing string (default)
# - verbose: Response + detailed metrics breakdown
_metrics_level = os.getenv("YAKK_METRICS_LEVEL", "summary").lower()
...
PREFER_LOCAL = os.getenv("YAKK_PREFER_LOCAL", "true").lower() in ("true", "1", "yes", "on")

# Always try local providers (don't mark them as permanently unavailable)
ALWAYS_TRY_LOCAL = os.getenv("YAKK_ALWAYS_TRY_LOCAL", "true").lower() in ("true", "1", "yes", "on")
...
CONCH_ENABLED = os.getenv("YAKK_CONCH_ENABLED", "true").lower() in ("true", "1", "yes", "on")

# Maximum time (seconds) to wait for conch when wait_for_conch=true
CONCH_TIMEOUT = float(os.getenv("YAKK_CONCH_TIMEOUT", "60"))

# How often (seconds) to check if conch is free when waiting
CONCH_CHECK_INTERVAL = float(os.getenv("YAKK_CONCH_CHECK_INTERVAL", "0.5"))
...
CONCH_LOCK_EXPIRY = float(os.getenv("YAKK_CONCH_LOCK_EXPIRY", "300"))
...
AUTO_FOCUS_PANE = env_bool("YAKK_AUTO_FOCUS_PANE", False)
...
TTS_BASE_URLS = parse_comma_list("YAKK_TTS_BASE_URLS", "http://127.0.0.1:8880/v1")
STT_BASE_URLS = parse_comma_list("YAKK_STT_BASE_URLS", "http://127.0.0.1:2022/v1")
TTS_VOICES = parse_comma_list("YAKK_VOICES", "af_sky")
TTS_MODELS = parse_comma_list("YAKK_TTS_MODELS", "tts-1")
STT_MODEL = os.getenv("YAKK_STT_MODEL", "whisper-1")
STT_MODELS = parse_comma_list("YAKK_STT_MODELS", "")

# STT prompt for vocabulary biasing (helps with specialized terminology)
STT_PROMPT = os.getenv("YAKK_STT_PROMPT", "")
...
def get_voice_preferences() -> list[str]:
    ...
    Uses the YAKK_VOICES configuration which is loaded from:
    1. Environment variables (highest priority)
    2. Project-specific .yakk.env files 
    3. Global ~/.yakk/yakk.env file
    4. Built-in defaults
...
# - YAKK_TTS_BASE_URLS (comma-separated list)
# - YAKK_STT_BASE_URLS (comma-separated list)
# - YAKK_VOICES (comma-separated list)
# - YAKK_TTS_MODELS (comma-separated list)
...
WHISPER_MODEL = os.getenv("YAKK_WHISPER_MODEL", DEFAULT_WHISPER_MODEL)
WHISPER_PORT = int(os.getenv("YAKK_WHISPER_PORT", "2022"))
WHISPER_LANGUAGE = os.getenv("YAKK_WHISPER_LANGUAGE", "auto")
WHISPER_MODEL_PATH = expand_path(os.getenv("YAKK_WHISPER_MODEL_PATH", str(Path.home() / ".yakk" / "services" / "whisper" / "models")))

# ==================== KOKORO CONFIGURATION ====================

# Kokoro-specific configuration
KOKORO_PORT = int(os.getenv("YAKK_KOKORO_PORT", "8880"))
KOKORO_MODELS_DIR = expand_path(os.getenv("YAKK_KOKORO_MODELS_DIR", str(BASE_DIR / "models" / "kokoro")))
KOKORO_CACHE_DIR = expand_path(os.getenv("YAKK_KOKORO_CACHE_DIR", str(BASE_DIR / "cache" / "kokoro")))
KOKORO_DEFAULT_VOICE = os.getenv("YAKK_KOKORO_DEFAULT_VOICE", "af_sky")
KOKORO_MAX_REQUESTS = int(os.getenv("YAKK_KOKORO_MAX_REQUESTS", "25"))
...
# systemd unit reads YAKK_MLX_AUDIO_HOST / YAKK_MLX_AUDIO_PORT
# from yakk.env so changes here are picked up at next service start.
MLX_AUDIO_HOST = os.getenv("YAKK_MLX_AUDIO_HOST", "127.0.0.1")
MLX_AUDIO_PORT = int(os.getenv("YAKK_MLX_AUDIO_PORT", "8890"))
...
CLONE_MODEL = os.getenv("YAKK_CLONE_MODEL", "mlx-community/Qwen3-TTS-12Hz-1.7B-Base-4bit")

# ==================== SERVICE MANAGEMENT CONFIGURATION ====================

# Auto-enable services after installation
SERVICE_AUTO_ENABLE = env_bool("YAKK_SERVICE_AUTO_ENABLE", True)

# ==================== SOUND FONTS CONFIGURATION ====================

# Sound fonts are enabled by default for audio feedback during tool calls
# Set YAKK_SOUNDFONTS_ENABLED=false to disable
SOUNDFONTS_ENABLED = env_bool("YAKK_SOUNDFONTS_ENABLED", True)
...
DISABLE_SILENCE_DETECTION = os.getenv("YAKK_DISABLE_SILENCE_DETECTION", "false").lower() in ("true", "1", "yes", "on")

# VAD (Voice Activity Detection) configuration
VAD_AGGRESSIVENESS = int(os.getenv("YAKK_VAD_AGGRESSIVENESS", "3"))  # 0-3, higher = more aggressive
SILENCE_THRESHOLD_MS = int(os.getenv("YAKK_SILENCE_THRESHOLD_MS", "1000"))  # Stop after 1000ms (1 second) of silence
MIN_RECORDING_DURATION = float(os.getenv("YAKK_MIN_RECORDING_DURATION", "0.5"))  # Minimum 0.5s recording
VAD_CHUNK_DURATION_MS = 30  # VAD frame size (must be 10, 20, or 30ms)
INITIAL_SILENCE_GRACE_PERIOD = float(os.getenv("YAKK_INITIAL_SILENCE_GRACE_PERIOD", "1"))  # No initial silence grace period by default

# Default listen duration for converse tool
DEFAULT_LISTEN_DURATION = float(os.getenv("YAKK_DEFAULT_LISTEN_DURATION", "120.0"))  # Default 120s listening time

# Repeat phrase detection for audio replay
REPEAT_PHRASES = parse_comma_list("YAKK_REPEAT_PHRASES", "repeat,say that again,pardon,what,come again")

# Wait phrase detection for pausing conversation
WAIT_PHRASES = parse_comma_list("YAKK_WAIT_PHRASES", "wait")

# Wait duration in seconds when wait phrase is detected
WAIT_DURATION = float(os.getenv("YAKK_WAIT_DURATION", "60.0"))  # Default 60s (1 minute)

# Audio feedback chime configuration
# Leading silence before chimes to allow Bluetooth devices to wake up
CHIME_LEADING_SILENCE = float(os.getenv("YAKK_CHIME_LEADING_SILENCE", "0.1"))  # Default 0.1s - minimal delay for Bluetooth
# Trailing silence after chimes to prevent cutoff
CHIME_TRAILING_SILENCE = float(os.getenv("YAKK_CHIME_TRAILING_SILENCE", "0.2"))  # Default 0.2s - reduced for responsiveness

# Audio format configuration
AUDIO_FORMAT = os.getenv("YAKK_AUDIO_FORMAT", "pcm").lower()
TTS_AUDIO_FORMAT = os.getenv("YAKK_TTS_AUDIO_FORMAT", "pcm").lower()  # Default to PCM for optimal streaming
...
STT_AUDIO_FORMAT = os.getenv("YAKK_STT_AUDIO_FORMAT", "mp3" if AUDIO_FORMAT == "pcm" else AUDIO_FORMAT).lower()
...
STT_SAVE_FORMAT = os.getenv("YAKK_STT_SAVE_FORMAT", "wav").lower()
...
STT_COMPRESS = os.getenv("YAKK_STT_COMPRESS", "auto").lower()
...
# Format-specific quality settings
OPUS_BITRATE = int(os.getenv("YAKK_OPUS_BITRATE", "32000"))  # Default 32kbps for voice
MP3_BITRATE = os.getenv("YAKK_MP3_BITRATE", "32k")  # Default 32kbps (optimal for speech per Whisper research)
AAC_BITRATE = os.getenv("YAKK_AAC_BITRATE", "64k")  # Default 64kbps

# ==================== STREAMING CONFIGURATION ====================

# Streaming playback configuration
STREAMING_ENABLED = os.getenv("YAKK_STREAMING_ENABLED", "true").lower() in ("true", "1", "yes", "on")
STREAM_CHUNK_SIZE = int(os.getenv("YAKK_STREAM_CHUNK_SIZE", "4096"))  # Download chunk size
STREAM_BUFFER_MS = int(os.getenv("YAKK_STREAM_BUFFER_MS", "150"))  # Initial buffer before playback
STREAM_MAX_BUFFER = float(os.getenv("YAKK_STREAM_MAX_BUFFER", "2.0"))  # Max buffer in seconds
...
TTS_TRAILING_SILENCE = float(os.getenv("YAKK_TTS_TRAILING_SILENCE", "0.5"))

# ==================== EVENT LOGGING CONFIGURATION ====================

# Event logging configuration
# Event logs are enabled by default, or if SAVE_ALL is true
EVENT_LOG_ENABLED = SAVE_ALL or os.getenv("YAKK_EVENT_LOG_ENABLED", "true").lower() in ("true", "1", "yes", "on")
EVENT_LOG_DIR = os.getenv("YAKK_EVENT_LOG_DIR", str(LOGS_DIR / "events"))
EVENT_LOG_ROTATION = os.getenv("YAKK_EVENT_LOG_ROTATION", "daily")  # Currently only daily is supported
...
def setup_logging() -> logging.Logger:
    """Configure logging for the yakk server.
    
    Returns:
        Logger instance configured for yakk
    """
    ...
    if TRACE_DEBUG:
        ...
        debug_log_dir = Path.home() / ".yakk" / "logs" / "debug"
        debug_log_dir.mkdir(parents=True, exist_ok=True)
        
        # Create dated debug log file
        debug_log_file = debug_log_dir / f"yakk_debug_{datetime.now().strftime('%Y-%m-%d')}.log"
        ...
        trace_file = Path.home() / "yakk_trace.log"
        trace_logger = logging.getLogger("yakk.trace")
        ...
        def trace_calls(frame, event, arg):
            if event == 'call':
                code = frame.f_code
                if 'yakk' in code.co_filename or 'yakk' in code.co_filename:
    ...
    if DEBUG:
        debug_log_file = Path.home() / "yakk_debug.log"
...
def initialize_directories():
    """Create necessary directories for yakk data storage."""
    ...
    # CONFIG_DIR.mkdir(exist_ok=True)  # Removed - config stored in .yakk.env file instead
...
def initialize_soundfonts():
    ...
    Directory structure:
        ~/.yakk/soundfonts/
            yakk/     - Package-managed soundfonts (synced from package)
            current -> yakk  - Relative symlink to active soundfont
    ...
    The 'yakk' directory is synced only when the package version changes.
    """
    from yakk.__version__ import __version__

    soundfonts_dir = BASE_DIR / "soundfonts"
    package_soundfont_dir = soundfonts_dir / "yakk"
    current_symlink = soundfonts_dir / "current"
    version_file = soundfonts_dir / ".version"

    # Migration: rename old 'default' directory to 'yakk'
        ...
        try:
            ...
            if current_symlink.is_symlink():
                ...
                if "default" in link_target:
                    ...
                    current_symlink.symlink_to("yakk")
        except OSError:
    ...
    if current_symlink.is_symlink():
        try:
            link_target = str(current_symlink.readlink())
            # If it's an absolute path pointing to yakk, make it relative
            if link_target.startswith("/") and link_target.endswith("/yakk"):
                current_symlink.unlink()
                current_symlink.symlink_to("yakk")
        except OSError:
            pass
    ...
    try:
        ...
        # Sync package soundfonts to 'yakk' directory incrementally
        # Only update files that are missing or different
        try:
            # For Python 3.9+
            from importlib.resources import files
            package_soundfonts = files("yakk.data.soundfonts.default")
            ...
            if pkg_resources.resource_exists("yakk", resource_dir):
                # This is a bit more complex with pkg_resources
                # We'll need to manually copy the structure
                pass

        # Create relative symlink to current soundfont (points to yakk)
        ...
        if DEBUG:
            import logging
            logging.getLogger("yakk").debug(f"Could not initialize soundfonts: {e}")
...
# Host/IP address to bind the server to (default: 127.0.0.1)
SERVE_HOST = os.getenv("YAKK_SERVE_HOST", "127.0.0.1")

# Port to bind the server to (default: 8765)
SERVE_PORT = int(os.getenv("YAKK_SERVE_PORT", "8765"))

# Allow connections from local/private IP ranges (default: true)
SERVE_ALLOW_LOCAL = env_bool("YAKK_SERVE_ALLOW_LOCAL", True)

# Allow connections from Anthropic IP ranges for Claude Cowork (default: false)
SERVE_ALLOW_ANTHROPIC = env_bool("YAKK_SERVE_ALLOW_ANTHROPIC", False)

# Allow connections from Tailscale IP range 100.64.0.0/10 (default: false)
SERVE_ALLOW_TAILSCALE = env_bool("YAKK_SERVE_ALLOW_TAILSCALE", False)

# Additional allowed CIDR ranges (comma-separated, default: empty)
SERVE_ALLOWED_IPS = os.getenv("YAKK_SERVE_ALLOWED_IPS", "")

# URL secret path segment for authentication (default: empty/disabled)
SERVE_SECRET = os.getenv("YAKK_SERVE_SECRET", "")

# Bearer token for authentication (default: empty/disabled)
SERVE_TOKEN = os.getenv("YAKK_SERVE_TOKEN", "")

# Transport protocol (streamable-http or sse)
SERVE_TRANSPORT = os.getenv("YAKK_SERVE_TRANSPORT", "streamable-http")

# ==================== THINK OUT LOUD CONFIGURATION ====================

# Enable Think Out Loud mode
THINK_OUT_LOUD_ENABLED = env_bool("YAKK_THINK_OUT_LOUD", False)
...
THINKING_STYLE = os.getenv("YAKK_THINKING_STYLE", "sequential")

# Whether to announce which voice is speaking
THINKING_ANNOUNCE_VOICE = env_bool("YAKK_THINKING_ANNOUNCE_VOICE", True)
