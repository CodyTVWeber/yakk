...
### 2. Import Updates
- Update any imports from `yakk.tools.services.{service}` to `yakk.tools.{service}`
...
- `yakk/tools/service.py` (imports `get_kokoro_version` from version_info)

**Decision**: Move to `yakk/utils/services/` to keep them as utilities rather than tools.

#### 2.2 Move Version Files
```bash
# Create utils/services directory if it doesn't exist
mkdir -p yakk/utils/services

# Move version files to utils
git mv yakk/tools/services/version_info.py yakk/utils/services/
git mv yakk/tools/services/list_versions.py yakk/utils/services/

# Update imports in service.py
# Change: from yakk.tools.services.version_info import get_kokoro_version
# To: from yakk.utils.services.version_info import get_kokoro_version
```
...
```bash
git add yakk/tools/__init__.py
...
git mv yakk/tools/services/whisper yakk/tools/
...
- `yakk/cli.py` (8 imports)
- `yakk/tools/services/whisper/__init__.py` (7 imports - now at tools/whisper/)
- `yakk/tools/services/whisper/model_*.py` files (4 files)
...
git mv yakk/tools/services/kokoro yakk/tools/
...
git mv yakk/tools/services/livekit yakk/tools/
...
ls -la yakk/tools/services/
# Should only show . and .. (empty)

# Remove the directory
rmdir yakk/tools/services
```

#### 5.2 Final Verification
```bash
# Verify structure
tree yakk/tools -d -L 1
...
git diff master...HEAD -- yakk/tools/__init__.py
```
...
```bash
# Update CLI imports
sed -i '' 's/from yakk\.tools\.services\.whisper/from yakk.tools.whisper/g' yakk/cli.py

# Update internal whisper imports
find yakk/tools/whisper -name "*.py" -exec \
  sed -i '' 's/from yakk\.tools\.services\.whisper/from yakk.tools.whisper/g' {} \;
```

#### Kokoro Import Updates
```bash
# Update CLI imports
sed -i '' 's/from yakk\.tools\.services\.kokoro/from yakk.tools.kokoro/g' yakk/cli.py

# Update service.py import
sed -i '' 's/from yakk\.tools\.services\.version_info/from yakk.utils.services.version_info/g' yakk/tools/service.py
```

#### LiveKit Import Updates
```bash
# Update CLI imports
sed -i '' 's/from yakk\.tools\.services\.livekit/from yakk.tools.livekit/g' yakk/cli.py

# Update service.py import
sed -i '' 's/from yakk\.tools\.services\.livekit/from yakk.tools.livekit/g' yakk/tools/service.py

# Update test imports
sed -i '' 's/from yakk\.tools\.services\.livekit/from yakk.tools.livekit/g' tests/test_livekit_*.py
```
